...
function initSwitchGallery() {
	var hiddenClass = 'js-gallery-hidden';
	var tabStyleSheet = $('<style type="text/css">')[0];
	var tabStyleRule = '.'+hiddenClass;
	var win = jQuery(window);

	jQuery('.switch-gallery-holder').each(function() {
		var holder = jQuery(this);
		var headGallery = holder.find('.head-gallery');
		var contentGalleries = holder.find('.summary-carousel');

		headGallery.scrollAbsoluteGallery({
			mask: '.mask',
			slider: '.slideset',
			slides: '.slide',
			btnPrev: 'a.btn-prev',
			...
			onInit: function() {
				...
				contentGalleries.not(contentGalleries.eq(this.currentIndex)).addClass(hiddenClass);
				win.on('load', function() {
					self.resizeHandler();
				});
			},
			onChange: function() {
				var currGallery = contentGalleries.eq(this.currentIndex);
				contentGalleries.addClass(hiddenClass);
				currGallery.removeClass(hiddenClass);
				if (currGallery.data('ScrollGallery')) {
					currGallery.data('ScrollGallery').onWindowResize();
					win.trigger('refreshSameHeight');
				}
			}
		});
	});

	tabStyleRule += '{position:absolute !important;left:-9999px !important;top:-9999px !important;display:block !important; width: 100%;}';
...
function initCarousel() {
	jQuery('.summary-carousel').scrollGallery({
		mask: '.mask',
		slider: '.slideset',
		slides: '.slide',
		btnPrev: 'a.btn-prev',
...
function initAccordion() {
	jQuery('ul.accordion').slideAccordion({
		...
		animSpeed: 300,
		animEnd: function() {
			jcf.refresh('#nav .jcf-scrollable');
		}
	});
}
...
function initSameHeight() {
	...
	jQuery('.summary-carousel').sameHeight({
		elements: '.frame',
		flexible: true,
		multiLine: true
	});
...
;(function($){
	...
	SlideAccordion.prototype = {
		...
		hideSlides: function(){
			...
			this.items.removeClass(this.options.activeClass);
			this.sliders.slideUp(this.options.animSpeed, function() {
				self.makeCallback('animEnd');
			});
		},
		makeCallback: function(name) {
...
;(function($) {
	...
	MobileNav.prototype = {
		...
		show: function() {
			...
			this.makeCallback('refreshScroll');
		},
		hide: function() {
...
;(function($){
	$.fn.sameHeight = function(opt) {
		...
		return this.each(function(){
			...
			if(options.flexible) {
				$(window).bind('resize orientationchange fontresize refreshSameHeight', delayedResizeHandler);
			}
